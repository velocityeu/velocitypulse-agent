 ...
 */
export async function performUpgrade(targetVersion, downloadUrl, logger) {
 ...
 */
async function windowsUpgrade(extractDir, installDir, targetVersion, logger) {
 ...
 */
async function linuxUpgrade(extractDir, installDir, targetVersion, logger) {
 ...
 */
export async function rollback(logger) {
    const installDir = getInstallDir();
    const backupDir = path.join(installDir, 'previous');
    try {
