...
export async function querySnmp(
  ...
  try {
    ...
    const result = await new Promise<SnmpInfo | null>((resolve) => {
      const timer = setTimeout(() => {
        session.close()
        resolve(null)
      }, timeout + 1000)

      session.get(oids, (error: Error | null, varbinds: Array<{ oid: string; value: Buffer | string; type: number }>) => {
        clearTimeout(timer)
        session.close()
        ...
        for (const vb of varbinds) {
          ...
          if (snmp.default.isVarbindError(vb)) continue

          const value = vb.value?.toString()?.trim()
          if (!value) continue

          if (vb.oid === SYSTEM_OIDS.sysDescr) { info.sysDescr = value; hasData = true }
          else if (vb.oid === SYSTEM_OIDS.sysName) { info.sysName = value; hasData = true }
          else if (vb.oid === SYSTEM_OIDS.sysContact) { info.sysContact = value; hasData = true }
          else if (vb.oid === SYSTEM_OIDS.sysLocation) { info.sysLocation = value; hasData = true }
        }
