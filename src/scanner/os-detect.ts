...
export async function detectOs(
  ...
  openPorts?: number[],
  services?: string[]
): Promise<OsDetectResult> {
  const hints: string[] = []
  let deviceType: OsDetectResult['device_type'] = 'unknown'
  ...
  if (ports.has(3389)) {
    if (!hints.includes('Windows')) hints.push('Windows')
    deviceType = 'workstation'
  }

  if (ports.has(22) && !ports.has(3389)) {
    if (!hints.some(h => h.includes('Linux'))) hints.push('Linux/Unix')
  ...
  if (ports.has(161) && !ports.has(22) && !ports.has(3389)) {
    if (!hints.includes('Network Equipment')) hints.push('Network Equipment')
    deviceType = 'network'
  }
 ...
 */
async function getTtl(ip: string, logger: Logger): Promise<number | null> {
  try {
