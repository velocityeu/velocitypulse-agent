 ...
 */
export function identifyService(banner: string, port: number): string | undefined {
  const lower = banner.toLowerCase()

  if (lower.startsWith('ssh-')) return 'ssh'
  if (lower.includes('220') && (lower.includes('ftp') || port === 21)) return 'ftp'
  if (lower.includes('220') && (lower.includes('smtp') || lower.includes('mail') || port === 25)) return 'smtp'
  ...
  if (lower.includes('microsoft') && lower.includes('sql')) return 'mssql'
  if (lower.includes('imap')) return 'imap'
  if (lower.includes('pop3') || lower.includes('+ok')) return 'pop3'
  if (lower.includes('telnet')) return 'telnet'
  if (lower.includes('vnc')) return 'vnc'
  if (lower.includes('apache') || lower.includes('nginx') || lower.includes('server:')) return 'http'
